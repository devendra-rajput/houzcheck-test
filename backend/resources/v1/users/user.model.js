...
const getAllWithPagination = async (page, limit, filterObj = {}) => {
  try {
    ...
    // Generate cache key
    const cacheKey = dataHelper.generateCacheKey(CACHE_KEYS.usersList, {
      page,
      limit,
      role: filterObj?.role || 'all',
      search: filterObj?.search || null,
    });
    ...
    if (filterObj?.search) {
      dbQuery = {
        ...dbQuery,
        [Op.or]: [
          { first_name: { [Op.like]: `%${filterObj.search}%` } },
          { last_name: { [Op.like]: `%${filterObj.search}%` } },
...
const getFormattedData = (userObj) => {
  ...
  return {
    ...
    organization: userObj?.organization || null,
